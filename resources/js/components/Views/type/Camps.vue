        ...
        >
          <v-expansion-panel>
            <v-expansion-panel-content
              v-for="(category, index) in product.typeCamps.categories"
              :key="index"
            >
              <v-checkbox
                v-model="checkFilter"
      ...
      <v-main>
       <v-container
        ...
        >
          <v-row>
            <v-col
              v-for="(search, index) in product.typeCamps.items.data"
              :key="index"
              cols="12"
              md="3"
            >
              <v-card
                  :href="search.pdo ? `/Lessons/${search.slug}/pdo/${search.pdo.id}` : '#'"
              >
                  <v-img
            ...
            >
              <template>
                <div class="text-center">
                  <v-pagination
                    v-if="product.typeCamps.items.last_page > 1"
                    v-model="page"
                    :length="product.typeCamps.items.last_page"
                    @input="paginate"
                  ></v-pagination>
                </div>
              </template>
...
export default {
    ...
    methods: {
      ...mapActions(["PRODUCT_BY_TYPE_CAMPS"]),
      ...
      paginate(e) {
        ...
        this.PRODUCT_BY_TYPE_CAMPS({type: 'Camps', filter:this.checkFilter, page, perPage: 12});
      },
      ...
      addFilter() {
        ...
        this.PRODUCT_BY_TYPE_CAMPS({type: 'Camps', filter: this.checkFilter, page: 1, perPage: 12});
      }
    },
